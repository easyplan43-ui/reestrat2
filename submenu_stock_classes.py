...
class ZalushTovary(QMainWindow):    #   Реалізує вкладку  - "Склад" -  "Залишок товару" 
    ...
    def show_table(self, index):   #  Виводить на екран поле таблицю з результатами пошуку
        ...
        rows = self.select_prod()   # беремо з бази сирий рядок даних 
        ...
        for row_index, row_data in enumerate(rows):
            for col_index, value in enumerate(row_data):
               item_text = str(value) if value is not None else ""   # Превращаем значение в строку для отображения
               item = QTableWidgetItem(item_text)
               self.table.setItem(row_index, col_index, item)  # Вставляем в ячейку       
    ...
    def select_prod(self):  #  Виводить з обох таблиць: main і depend інфу в таблицу на екрані
        cortege_fk_prim_key = self.ekzemp_category.get_foreignkey(self.table_name_depend)  # отримуємо 1 - назву стовп з власт foreign key, 2- назву стовпця на який цей foreign key ссилається
        ...
        query = f"""SELECT {all_columns} FROM {Table_main_product} AS m INNER JOIN {self.table_name_depend} AS d ON m.{cortege_fk_prim_key[0][1]} = d.{cortege_fk_prim_key[0][0]}; """
        try:
