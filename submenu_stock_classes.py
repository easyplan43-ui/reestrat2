...
class ZalushTovary(QMainWindow):    #   Реалізує вкладку  - "Склад" -  "Залишок товару" 
    ...
    def show_table(self, index):   #  Виводить на екран таблицю з результатами пошуку і поле пошуку з кнопкою Пошук
        ...
        self.table.setSortingEnabled(False)    # Вимикаємо сортування на час заповнення, щоб програма не "гальмувала"
         # ------------------------------------- Заповнення таблиці даними -----------------------------------------
        self.table.setRowCount(len(rows)) # Устанавливаем количество строк
        for row_index, row_data in enumerate(rows): # enumerte дає: 0 ('SAS-SG-360', 'Диск SAS Seagate 360 ', '2 SFF', 5, Decimal('87.00'), datetime.datetime(2026, 3, 31, 17, 16, 17, 570000), 'SAS-2', 360, 6000) 1 () 2 (),....
            for col_index, value in enumerate(row_data):  # 0 NVME-WD-720  1 Диск NVME Western Digital 720  2 PCIe 40  3 11  4 231.00 5 2026-03-31 04:43:06.980000
               komirka = QTableWidgetItem()  # беремо текст (або число) і загортає його в комірку, який "розуміє" таблиця типу: <PyQt6.QtWidgets.QTableWidgetItem object at 0x0000027F6CA20C30>
               ...
               self.table.setItem(row_index, col_index, komirka)  # Вставляем в ячейку 
        self.table.setSortingEnabled(True)        
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Interactive)
        self.table.horizontalHeader().setSortIndicatorShown(True)  # Додаємо відображення стрілочок сортування
    ...
    def check_searched_text(self):  #  Робить провірку введеного тексту користувач і якщо не пустий текст викликає інший метод для пошуку тексту в бд
        ...
        if not vvedenuy_text:  # Якщо поле порожнє — можна вивести попередження
            ...
            msg.setText("Поле пошуку не може бути порожнім!")
            msg.setIcon(QMessageBox.Icon.Warning)   # Жовтий знак оклику
