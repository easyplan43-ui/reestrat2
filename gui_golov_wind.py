...
class MainWindow(QMainWindow):  # Работает как «сборщик»
    def __init__(self, user_permission, user_group_access, full_username):
        ...
        self.submenu = SubmenuBase(self)   # створюємо підменю від головних кнопок
...
class SubmenuBase(QFrame):    # Створює підменю від головного лівостронього toolbar c копками
    ...
    def create_dynamic_submenu(self, dict_name_btns_submenu):    #  На вхід приймає словник за категор. типу: Назва_кнопки_підменю: Назва_класу_що_її_обробляє
        ...
        for item in dict_name_btns_submenu:       #  Проходимся по: Залишки товарів,  Переміщення, Інвентаризація, Списання, ...
            ...
            btn.clicked.connect(lambda checked, b=btn: self.create_nested_submenu(b)) #  сигнал clicked всегда отправляет логическое значение (True/False),
               # b=button «замораживает» текущий объект кнопки внутри lambda в момент создания связи
    ...
    def create_nested_submenu(self, clicked_button):
        ...
        if not clicked_button.isChecked():             # Якщо кнопку щойно вимкнули (зняли чек), просто виходимо після очищення
           return
        btn_text = clicked_button.text()    # Отримуємо текст / название самой кнопки типу: Внесення товару
        if btn_text == 'Внесення товару':
           index = self.layout.indexOf(clicked_button)    # Знаходимо в серед layout, поряд номер натиснут кнопки
           ekzemp_category = SelectCategory('hardware.Categories')  # Створюємо екземпляр класу, передаючи назву таблиці
           category = ekzemp_category.get_category() # Викликаємо метод get_category, який повертає список кортежів Категорії товарів з табл Categories типу [('Процесори',), ('Накопичувачі',), ('Оперативна память',), ('Материнська плата',)]
           nested_items = [item[0] for item in category]  # Генерат списку, перетвор з списку кортежів в список типу ['Процесори', 'Накопичувачі', 'Оперативна память', 'Материнська плата']
             
           # 4. Вставляємо нові кнопки ОДНУ ЗА ОДНОЮ під натиснуту кнопку
           for i, item_text in enumerate(nested_items):
              sub_btn = QPushButton(f"   ↳ {item_text}")    # Формуємо кнопки під-підменю
              sub_btn.setStyleSheet(NestedmenuStyle.SUB_ITEM)    # Застосовуємо стиль з окремого класу 
              sub_btn.clicked.connect(self.add_tab) # add_tab без дужок це реєстрація функ яка виконається коли нажати на кнопку 
    ...
    def add_tab(self):              # Добавляє вкладки в основ робочу область, но предотвращает открытие нескольких одинаковых окон
        # Отримуємо об'єкт кнопки, яка викликала цей метод
        sub_btn = self.sender()   # Отримуємо об'єкт кнопки, яка викликала цей метод
        btn_text = sub_btn.text()   #  определяем текст кнопки submenu, на которую нажал пользователь, наприклад  - Особисті картки
...
class NestedmenuStyle:   # Стиль вкладеного під-підменю
    PANEL = "background-color: #f0f0f0; border-right: 1px solid #bdc3c7;"
    HEADER = "font-weight: bold; padding: 10px; background-color: #dcdde1;"
    SUB_ITEM = """
        QPushButton {
            text-align: left;
            ...
            /* ВІДСТУПИ ЗЗОВНІ (зменшують рамку) */
        margin-left: 30px;   /* відступ зліва (щоб була ієрархія) */
        margin-right: 10px;  /* відступ справа */
        margin-top: 2px;     /* відступ зверху */
        margin-bottom: 2px;  /* відступ знизу */
            background: transparent;
        }
        QPushButton:hover {   background-color: #e8e8e8; border: 1px solid #aaaaaa; /* колір при наведенні мишкою */  }
        QPushButton:pressed {  background-color: #d0d0d0;  border: 1px solid #888888;/* ТЕМНІШИЙ колір при натисканні */  }
    """
